<template>
  <div class="Main">
    ...
    <right-menu @changeSettingVisible="changeSettingVisible" @zoom="zoomMap"/>
  </div>
</template>
  ...
  methods: {
    ...
    flightSocket() {
      ...
      this.airSocket.onopen = () => {
        ...
        let isDebug = JSON.stringify({is_debug: 1})
    ...
    getFlightRouter(info) {
      getFlightDetail({icao: info.icao, generated_date: info.generated_date, is_debug: 1}).then(res => {
        if(res.data.status === 200) {
