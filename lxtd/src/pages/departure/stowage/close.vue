    ...
    <el-row>
      <el-col :span="6" :offset="1">
        <div class="grid-content bg-purple">
          <el-button type="primary" size="mini" @click="closeFlight">航班关闭</el-button>
        </div>
      </el-col>
  ...
  <div class="receive_list">
    ...
    element-loading-background="rgba(0, 0, 0, 0.8)"
     @selection-change="getList">
      ...
      <el-table-column sortable prop="sender_telephone" label="航线">
...
<script>
  import { getFlightList, closeFlight } from "@/service/stowage.js"
  ...
  export default {
    ...
    methods: {
      ...
      search() {
        ...
        getFlightList({...this.searchInfo.params, currPage: this.currPage, pageSize: this.pageSize}).then(res => {
      ...
      close() {
        let flightList = this.list.map(item => item.id)
        let params = {flightList: flightList}

        closeFlight(params).then(res => {
          if(res.data.status === 200) {
            this.$notify({
              title: "成功",
              message: "航班关闭成功",
            ...
            this.$store.commit("CHANGE_STOWAGESTEP", 5)
            this.search()
          }
        })
      },
